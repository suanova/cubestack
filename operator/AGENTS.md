...
The static content of `helm/cubestack-operator/` (`templates/`, `crds/`, `vap.yaml`) is generated by `hack/update-helm-resources.sh` from the `config/` sources (plus the lws CRDs from the pinned `sigs.k8s.io/lws` go module). Only `Chart.yaml`, `values.yaml`, `.helmignore` and READMEs are hand-written.
...
- Never hand-edit generated files — a future regeneration overwrites them and the CI `helm-drift` job (`make helm-resources-check`) fails on the diff.
...
make helm-resources-update  # Regenerate helm/cubestack-operator from config/ (commit with the change)
```
